"""
Simulation agent — wraps a persona, compiled phenotype, and LLM client
into an interactive agent with both sync and async interfaces.
...
class SimulationAgent:
    ...
    def generate_reply(self, post_content: str, author_name: str) -> str:
        """Generate a reply to another user's post."""
    ...
    def should_engage(self, post_content: str, author_name: str) -> bool:
        """Decide whether this persona would engage with a given post."""
        user_prompt = (
    ...
    def generate_media_reaction(self, media_item: MediaItem) -> str:
        """Generate a reaction/post in response to a media item."""
        user_prompt = (
            f'You are reading an article titled "{media_item.title}".\n\n'
            f"Article content:\n{media_item.content}\n\n"
            f"Write a post sharing your reaction, thoughts, or commentary on this article."
        )
    ...
    async def generate_post_async(self, topic: str = None) -> str:
        """Async version of generate_post."""
        user_prompt = "Draft a new post for your followers."
        if topic:
            user_prompt += f" The current trending topic is: {topic}."
